...
public final class SelfUpdateLog: @unchecked Sendable {
    ...
    /// Truncates, rather than appending forever: one self-update per file, which is what anyone reading it
    ...
    private func writeToStandardError(_ message: String) {
        FileHandle.standardError.write(Data("HomebrewUpdateHelper: \(message)\n".utf8))
    }
