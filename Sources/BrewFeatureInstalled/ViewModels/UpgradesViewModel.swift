...
final class UpgradesViewModel {
    ...
    /// Top-level filter: hides outdated packages that another installed package depends on.
    /// Client-side, alongside the scope picker and search query; never refetches.
    ...
    /// True while any filter — the scope picker, the top-level toggle or the search field — is
    /// narrowing the outdated inventory.
    var isFiltering: Bool {
        isSearchActive || scope != .all || showsTopLevelPackagesOnly
    }
    ...
    /// Committed selections are left intact — `activeSelectedPackageID` falls back to the first visible
    /// row while a selection is filtered out, and restores it if the filters widen again. Mirrors
    /// `InstalledViewModel`.
...
extension UpgradesViewModel {
    ...
    /// Subtitle while a filter is narrowing the list: "Showing N of M upgrades", or
...
extension UpgradesViewModel {
    ...
    var bulkUpgradeSummary: String {
        ...
        case let .explicit(names):
            if showsTopLevelPackagesOnly, !isSearchActive {
                return Self.topLevelUpgradeSummary(count: names.count)
            }
            return Self.searchedUpgradeSummary(count: names.count)
        }
    }
    ...
    private static func topLevelUpgradeSummary(count: Int) -> String {
        if count == 1 {
            return String(
                localized: "Upgrades the 1 top-level package",
                comment: "Upgrades header command summary for a single top-level package",
            )
        }
        return String(
            localized: "Upgrades the \(count) top-level packages",
            comment: "Upgrades header command summary when the top-level filter narrows the batch",
        )
    }
    ...
    /// - The top-level filter does the same — `brew upgrade` and the kind flags cannot express
    ///   "only packages nothing depends on", so the visible rows are named explicitly.
    /// - Otherwise the scope picker maps to everything / `--formula` / `--cask`.
    var upgradeSelection: BrewUpgradeSelection {
        if isSearchActive || showsTopLevelPackagesOnly, !allRows.isEmpty {
    ...
    /// Clears every filter, restoring the full outdated list. Wired to the "Show all upgrades"
    /// affordance on the no-matches empty state.
    func resetFilters() {
        searchQuery = ""
        scope = .all
        showsTopLevelPackagesOnly = false
    }
}
