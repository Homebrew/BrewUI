//
//  SelfUpdateHelperRunTests.swift
...
struct SelfUpdateHelperRunTests {
    // MARK: The app exits
    ...
    @Test func `an app that exits on the deadline is not treated as a timeout`() async {
        ...
        let result = await SelfUpdateHelperRun(
...
private final class EffectRecorder: @unchecked Sendable {
    ...
    var effects: SelfUpdateHelperRun.Effects {
        SelfUpdateHelperRun.Effects(
