...
struct SelfUpdateLogTests {
    private func makeFileURL() -> URL {
        URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("self-update-log-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func `lines are written in order`() throws {
        ...
        let log = SelfUpdateLog(fileURL: fileURL)
        log.begin()
        log.write("first")
        log.write("second")
        log.end()
    ...
    /// One self-update per file: after a failure, the previous run's output would only be confusing.
    @Test func `a second run replaces the previous log rather than appending`() throws {
        let fileURL = makeFileURL()
        defer { try? FileManager.default.removeItem(at: fileURL.deletingLastPathComponent()) }

        let first = SelfUpdateLog(fileURL: fileURL)
        first.begin()
        first.write("from the first run")
        first.end()

        let second = SelfUpdateLog(fileURL: fileURL)
        second.begin()
    ...
    @Test func `every line carries a timestamp`() throws {
        ...
        let log = SelfUpdateLog(fileURL: fileURL, clock: { fixed })
        log.begin()
        log.write("stamped")
        log.end()
    ...
    @Test func `an unopenable path does not trap the caller`() {
        let log = SelfUpdateLog(fileURL: URL(fileURLWithPath: "/dev/null/not-a-directory/self-update.log"))
        log.begin()
    ...
    @Test func `a nil file URL writes nothing and still accepts messages`() {
        let log = SelfUpdateLog(fileURL: nil)
        log.begin()
