//
//  SelfUpdateUpgradeRunnerTests.swift
//  BrewSelfUpdateHelperCoreTests
//

@testable import BrewSelfUpdateHelperCore
import Foundation
import Testing
...
struct SelfUpdateUpgradeRunnerTests {
    // MARK: Exit status
    ...
    @Test func `both streams reach the transcript, in the order they were written`() async throws {
        ...
        let outcome = await SelfUpdateUpgradeRunner(transcriptSink: { transcript.append($0) }).run(
            executablePath: brew.path,
            arguments: [],
            environment: [:],
            timeout: 30,
    ...
    @Test func `output that never ends in a newline is still captured`() async throws {
        ...
        let transcript = TranscriptRecorder()
        _ = await SelfUpdateUpgradeRunner(transcriptSink: { transcript.append($0) }).run(
            executablePath: brew.path,
            arguments: [],
            environment: [:],
            timeout: 30,
    ...
    @Test func `pinned environment variables reach brew`() async throws {
        ...
        _ = await SelfUpdateUpgradeRunner(transcriptSink: { transcript.append($0) }).run(
            executablePath: brew.path,
            arguments: [],
            environment: ["BREW_UITEST_SCENARIO": "selfUpdateAvailable"],
            timeout: 30,
        )

        #expect(transcript.lines == ["selfUpdateAvailable"])
    }

    @Test func `the inherited environment survives alongside the pinned one`() {
        let environment = SelfUpdateUpgradeRunner.environment(adding: ["PINNED": "yes"])
    ...
    @Test func `colour is switched off for the transcript`() {
        let environment = SelfUpdateUpgradeRunner.environment(adding: [:])
    ...
    @Test func `a pinned value wins over the defaults`() {
        let environment = SelfUpdateUpgradeRunner.environment(adding: ["HOMEBREW_NO_COLOR": "0"])
...
private struct FakeExecutable {
    ...
    static func runner() -> SelfUpdateUpgradeRunner {
        SelfUpdateUpgradeRunner()
    }
}
