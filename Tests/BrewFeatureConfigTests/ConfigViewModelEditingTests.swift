...
struct ConfigViewModelEditingTests {
    ...
    @Test @MainActor func `sections(for:) is cache-independent so AsyncContentView placeholders render`() {
        let viewModel = ConfigViewModel(
            repository: StubConfigRepository(state: .loading),
            envFileRepository: StubEnvFileRepository(state: .loading),
            processEnvironment: [:],
    ...
    @Test @MainActor func `draft is seeded from the cached env file on init`() {
