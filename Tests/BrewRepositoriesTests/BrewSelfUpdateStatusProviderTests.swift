//
//  BrewSelfUpdateStatusProviderTests.swift
//  BrewTests
//
...
@MainActor
struct BrewSelfUpdateStatusProviderTests {
    private func makeProvider(
        packages: [InstalledBrewPackage],
        runningVersion: String = "1.4.2",
    ) -> BrewSelfUpdateStatusProvider {
        BrewSelfUpdateStatusProvider(
    ...
    private func appCask(
        ...
        .fixture(
            name: SelfUpdateIdentity.caskToken,
            kind: .cask,
            homepage: homepage,
    ...
    @Test func `outdated app cask surfaces an available update with both versions`() {
        ...
        let status = provider.selfUpdateStatus
    ...
    @Test func `a formula sharing the cask token name is not mistaken for the app`() {
        let provider = makeProvider(packages: [
            .fixture(name: SelfUpdateIdentity.caskToken, kind: .formula, outdated: true),
        ])
        #expect(provider.selfUpdateStatus.isUpdateAvailable == false)
    }
    ...
    @Test func `empty latest version maps to nil rather than an empty string`() {
        ...
        #expect(provider.selfUpdateStatus.latestVersion == nil)
    }
    ...
    @Test func `blank homepage falls back to the identity homepage`() {
        ...
        #expect(provider.selfUpdateStatus.homepageURL == SelfUpdateIdentity.homepageURL)
    }
...
@MainActor
struct BrewSelfUpdateDetectionIntegrationTests {
    ...
    private func provider(appOutdated: Bool) async -> BrewSelfUpdateStatusProvider {
        ...
        return BrewSelfUpdateStatusProvider(
            inventory: repo,
    ...
    @Test func `outdated app cask in a real payload surfaces an available update`() async {
        let status = await provider(appOutdated: true).selfUpdateStatus
        #expect(status.isUpdateAvailable)
        #expect(status.runningVersion == "1.4.2")
        #expect(status.latestVersion == "1.5.0")
    ...
    @Test func `current app cask in a real payload reports no update even when other casks are outdated`() async {
        let status = await provider(appOutdated: false).selfUpdateStatus
        #expect(status.isUpdateAvailable == false)
        #expect(status.latestVersion == "1.5.0")
    }
}
