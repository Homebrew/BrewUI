...
private final class LocalizedCopyVisitor: SyntaxVisitor {
    ...
    override func visit(_ node: FunctionCallExprSyntax) -> SyntaxVisitorContinueKind {
        ...
        if LocalizedCopyRule.localizedInitializers.contains(callee.name) {
            ...
            if declaresKey, !carriesBundleAndComment(arguments) {
            ...
            if let first, first.label == nil, isCopyLiteral(first.expression),
               !carriesBundleAndComment(arguments)
            {
                context.record(rule: LocalizedCopyRule.self, at: node)
            }
    ...
    /// `bundle: .main` reads the app bundle, where a package key is not, so the argument has to be
    /// `#bundle` rather than merely present.
    private func carriesBundleAndComment(_ arguments: LabeledExprListSyntax) -> Bool {
        guard arguments.contains(where: { $0.label?.text == "comment" }),
              let bundle = arguments.first(where: { $0.label?.text == "bundle" })
        else {
            return false
        }
        return bundle.expression.as(MacroExpansionExprSyntax.self)?.macroName.text == "bundle"
    }
    ...
    override func visit(_ node: MemberTypeSyntax) -> SyntaxVisitorContinueKind {
        if node.name.text == "LocalizedStringKey", node.namesAModuleMember {
            context.record(rule: LocalizedCopyRule.self, at: node)
        }
        return .visitChildren
    }
    ...
    private func calleeName(_ expression: ExprSyntax) -> Callee? {
        ...
        if let member = expression.as(MemberAccessExprSyntax.self) {
            return Callee(name: member.declName.baseName.text, isMember: member.callsThroughValue)
        }
        return nil
    }
