...
/// Only UI packages may hold user-facing copy. Core, CLI, networking and repository layers throw
/// typed error enums (`BrewCore` / `BrewRepositoryInterfaces`) and `BrewUIComponents/Copy` words them.
struct LocalizationLayerRule: Rule {
    ...
    /// Repo-relative directory prefixes that may localise. Matched against any path suffix so absolute
    /// paths from CI and relative ones from the build plugin both work.
