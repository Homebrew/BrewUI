...
## 2026-09-15 — Localisation: per-target String Catalogs, English keys, copy only in UI packages

- **One `Localizable.xcstrings` per UI target** (`BrewUIComponents`, the six `BrewFeature*` packages, the `Homebrew` app), not one shared catalog: Xcode's extraction writes strings into the catalog of the target the code lives in, so any other layout stops syncing on build. `Package.swift` declares `defaultLocalization: "en"`.
- **`#bundle` at every call site.** A package string without a bundle is looked up in `Bundle.main` and never localises — the bug the pre-existing `String(localized:)` calls all had. In a target without resources `#bundle` is a compile error, which is the structural half of the layer rule.
- **English copy is the key; no generated symbols.** `STRING_CATALOG_GENERATE_SYMBOLS` only works under xcodebuild and `scripts/test` uses `swift test`. A changed English string becomes a new key (old one marked stale), which is what makes it show as untranslated everywhere.
- **Copy only in UI packages.** `BrewCore`/`BrewCLI`/`BrewNetworking`/`BrewRepositoryInterfaces`/`BrewRepositories` throw typed enums; `BrewUIComponents/Copy/BrewErrorCopy` words them, replacing four duplicated `userMessage(for:)` switches. Enforced by BrewUILint `localization_layer` and `brew-localize verify`.
- **`Tools/BrewLocalize` uses the compiler's extraction, not `xcstringstool extract`.** The lightweight extractor writes `%arg` where the runtime key has `%lld`/`%@`, so interpolated translations would never match. `sync` runs `swift build` with `-emit-localized-strings` into `.build/localize` and compiles the app sources with `swiftc -c` mirroring the Xcode target's flags (`-DDEBUG`, MainActor default isolation, MemberImportVisibility). Anything that only exists under `DEBUG` (debug menu, previews) therefore uses `Text(verbatim:)`, or Release builds would mark it stale.
- **An empty translation is not a fallback.** `xcstringstool compile` emits `"Key" = ""` and Foundation returns the empty string; `verify` rejects empty values and `status` counts them as untranslated.
- **`swift test` never sees translations** (SwiftPM copies the raw catalog; only `en` resolves), so package presentation tests assert English literals safely; the Xcode test plans pin `language: en` for the same reason.
- **The app catalog must carry each language** — macOS lists a language in the per-app picker only if the main bundle ships an `.lproj`. `en-GB` is seeded (Licence, catalogue, plus identical sidebar titles in the app catalog) so the picker and partial fallback are exercised on a real build. Source `en` is US spelling.
- **Maintainer signal is the CI job summary**, not build warnings: `brew-localize status --markdown` in `swift_quality`. Catalog drift is the only blocking check (`sync --check`, also in pre-commit when Swift or catalogs are staged).
