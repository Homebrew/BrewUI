...
- **An app cannot replace its own bundle while running, so the update is a handoff.** `HomebrewUpdateHelper` is a new Xcode target bundled at `Contents/Helpers/`, launched by `HelperSelfUpdateHandoff` just before `NSApplication.terminate` (in that order — the helper polls for the pid to disappear). Ordering inside the helper is **wait for exit → update → relaunch**; a wait that times out relaunches *without* updating. The contract travels as a JSON file path, not argv: a UI-test relaunch environment carries the whole fixture tree and would blow `ARG_MAX`. `BrewSelfUpdateContract` is dependency-free on purpose, so nothing in the app's module graph can leave the helper unable to bring the app back.
