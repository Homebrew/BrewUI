...
## 2026-09-07 — On-disk storage is split by recoverability and namespaced by bundle identifier

- **The app is unsandboxed, so `~/Library` is shared ground.** There is no container to namespace writes, and every store used to resolve its own path inline into a directory called `Brew` — a name that collides with anything else of that name and reads as if it belonged to the `brew` CLI rather than to this app. `BrewAppStorage` now names the directories once; both roots carry `sh.brew.app`.
- **Which root a store uses follows from whether its contents can be rebuilt, not from habit.** The catalogue and Discover analytics stores hold ETag-validated copies of HTTP responses, so they live in `Caches`: the system may purge them, they are excluded from Time Machine, and a purge costs one refetch. Crash reports live in `Application Support` because a pending report cannot be regenerated and a purge would destroy it before the user ever saw the prompt to file it. Do not "tidy" the two back into one root.
- **`bundleIdentifier` is spelled out rather than read from `Bundle.main`.** The crash-signal path and the unit tests must resolve the same directories the app does, and `Bundle.main` under `swift test` is the test runner. It is asserted against `PRODUCT_BUNDLE_IDENTIFIER` by a test so the two cannot drift.
- **`BrewAppStorage` is dependency-free and knows nothing about crash reports.** `BrewLegacyStorageMigration.run(preserving:)` takes the subdirectory names to rescue; `BrewApp` passes `CrashReportStore.directoryName`. Keeping the target at the bottom of the graph is what lets `BrewCrashReporting` depend on it without pulling in `BrewCore`.
- **The migration merges and only then deletes.** It moves files individually rather than moving the folder, because the destination may already hold reports from a newer launch — a name on both sides keeps the newer copy. Nothing is removed until every rescue succeeds, so a failure leaves the legacy directory for the next launch to retry. It returns whether the directory is gone rather than throwing: there is no user-facing failure here and nothing a caller could usefully do.
- **It is skipped under `-uiTesting`.** The migration deletes a real directory in the user's Library; a test run on a developer or CI machine must never touch the legacy folder of the real install sitting beside it. Same isolation rule as the fixture-backed cache seams.
- **The "attach the full crash log from …" note composes the path from those constants.** It was a literal, and a literal is exactly how a user-facing path goes stale the next time the directory moves.
