...
- **The app is unsandboxed, so `~/Library` is shared ground.** There is no container to namespace writes, and every store used to resolve its own path inline into a directory called `Brew` — a name that collides and reads as if it belonged to the `brew` CLI. `BrewAppStorage` names both roots once, each namespaced `sh.brew.app`.
- **Which root a store uses follows from whether its contents can be rebuilt.** Catalogue and Discover analytics hold ETag-validated HTTP responses, so they live in `Caches` (purgeable, excluded from Time Machine, one refetch to replace). Crash reports live in `Application Support` because a pending report cannot be regenerated and a purge would destroy it before the user saw the prompt to file it. Do not "tidy" the two back into one root.
- **`bundleIdentifier` is spelled out, not read from `Bundle.main`.** The crash-signal path and the unit tests must resolve the same directories the app does, and `Bundle.main` under `swift test` is the test runner. A test pins it to `PRODUCT_BUNDLE_IDENTIFIER`.
- **The migration merges, and only then deletes.** It moves files individually because the destination may hold newer reports; nothing is removed until every rescue succeeds, so a failure retries next launch. It is skipped under `-uiTesting`, which must never delete the legacy folder of the real install on the test machine.
