...
- **Doctor data layer:** `DoctorReport`/`DoctorIssue`/`DoctorSuggestedFix` (`BrewCore/Models/DoctorReport.swift`) + pure `DoctorOutputParser` (`BrewCore/Support/`). Parser splits `brew doctor` output into warning blocks (delimited by `Warning:` lines, blank lines allowed inside a block), treats whitespace-free body lines as affected items (kegs/paths), and extracts a runnable `brew` fix from a standalone `brew …` line or a backticked `` `brew …` `` — completing a verb-only command (e.g. `brew link`) with the affected items.
- **`DoctorRepository` is an app-scoped `@Observable @MainActor` port** (mirrors `InstalledInventoryObserving`/`BrewInstalledPackagesRepository`), NOT a stateless one-shot — it holds `state: LoadState<DoctorReport, any Error>` + `isRefreshing` and exposes `load()`. Long-lived (injected once in `BrewApp`) so the report **persists across leaving/returning to the Doctor tab**. `load()` is **stale-while-revalidate**: an existing report stays on screen (`isRefreshing` flips on) while the re-check runs; only the first load shows `.loading`; a failed refresh keeps the prior report (logs). Reads `brew doctor` read-only (NOT via the command center); **a non-zero exit means warnings were found, not a failure**, so the exit code is ignored and both stdout+stderr are parsed. Concurrent `load()`s coalesce onto one in-flight `Task`.
- **`BrewFeatureDoctor`** module (mirrors `BrewFeatureInstalled`): `DoctorViewModel` **projects** the repository's state into `DoctorPresentation` (`.loading`/`.healthy`/`.issues`/`.failed`) — no idle/CTA. `DoctorColumns` **auto-runs the check on arrival** via `.task { await viewModel.load() }`; the header shows a small spinner + "Re-checking…" while a background refresh runs. `runFix` submits a `.maintenance` op through the shared command center (output streams in the existing bottom console) and re-runs the repo's `load()` on success so resolved issues clear. Wired into the app via a `stethoscope` sidebar entry, a `MainWindowView` case, `BrewApp` injecting `BrewDoctorRepository.live()`, and a `BrewFeatureDoctor` product linked in `Brew.xcodeproj`.
...
- **`DoctorIssue` model redesigned around layered classifier sections** (`.ai/plans/DoctorParsing-Plan.md` + `-Addendum.md`): `severity` (from tier callout / `Unsupported configuration:`), `details` (un-indented prose residue), `affectedItems` (data blocks), `fixSequences` (consecutive indented commands grouped into ordered runs; each step carries `needsAdmin`), `inlineChips` (backticked `brew …` refs from prose — copy-only, never promoted to the primary fix rail), `links` split into `action` / `reference` by host (`developer.apple.com` / `macports.org` are actions; everything else reference), and `rawBody` as the verbatim fallback. Special-case un-indented item collection only for `do not exist:` / `not writable by your user:` (those checks emit dir lists at column 0). **Title can itself be the cue** (e.g. `check_access_directories`'s `Warning: The following directories are not writable…`) — parser arms from the title in that case.
- **Block classification is by first member, not per-line allowlist** (per the addendum, supersedes the earlier per-line data-intro cue set). A colon-terminated un-indented line opens a *pending block*; its first indented member decides whether the whole block is `.commands` / `.data` / `.links` / `.prose`. The allowlist is consulted once per block (for the first-member command test) plus as a `.prose` fallback for stray commands under period-ending prose (e.g. `check_git_status`'s `git stash`). The only surviving intro cue is `dataNounCue` — a hard guard that forces `.data` *before* the first-member command test for lists whose first item could read as a command (a deprecated formula named `git`/`python`/`node`). Keys on list-nouns (plural: `tools`/`formulae`/`casks`/`taps`/`directories`/`kegs`/`places`/`files`/`paths`) plus specific phrases (`the same name as`, `not readable:`, `on these:`, `out of the way:`) — must NOT match singular `following command:`. An `isValueLine` guard (`contains(" = ")` or `contains(": ")`) keeps key/value lines (`core.autocrlf = true`, `which resolves to: /path`) out of Affected when the block is data. The executable allowlist includes `echo` / `fish_add_path` / `set` for the shell-profile PATH fixes.
- **Run Fix is intentionally narrow.** `DoctorFixSequence.isRunnable` is true only for a single-step, non-admin `brew` step; multi-step sequences and anything needing `sudo` are copy-only (with a "Needs admin · runs in Terminal" hint above the block). The detail surface still shows one shared `Run Fix` button bound to `primaryRunnableSequence` — there is no synthesis of `brew <verb> <items>` from a chip + Affected items (intentional per the plan; users copy the chip + items if needed).
