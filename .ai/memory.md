...
- **`appLanguage` alone does not select framework localization.** `ApplicationEntry` applies the app-domain `AppleLanguages` before `BrewApp` exists. `UserDefaults(suiteName:)` cannot take the app's own bundle identifier; use `.standard` for that domain. Active copy follows `Bundle.main.preferredLocalizations`.
- **Reopen must not kill brew.** Idle reopen is the default; a running operation disables it. `SerialBrewCommandCenter.beginRelaunch` locks an idle center so an install cannot start after the check. The helper is the same executable waiting for the old PID — not the self-upgrade helper.
- **The confirmation is a free-standing panel, not a sheet.** Center it on the current window and order it front before `runModal`, or AppKit recenters it on the screen.
- **One catalog, three locales.** en / zh-Hans / zh-Hant in `Homebrew/Localizable.xcstrings`. Raw brew diagnostics stay in `AppMessage` untranslated. Configuration is 配置 because ConfigView is a brew config report, not app Settings. Tests isolate a defaults domain and pass its language as launch arguments on relaunch.
