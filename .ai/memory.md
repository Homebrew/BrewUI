...
- **`DoctorIssue` model redesigned around layered classifier sections** (`.ai/plans/DoctorParsing-Plan.md`): `severity` (from tier callout / `Unsupported configuration:`), `details` (un-indented prose residue), `affectedItems` **anchored to a closed data-intro cue set** (`on these:`, `Unexpected …:`, `following …:`, `… files:`, `Paths found:`, `Uncommitted files:`, `in the following places:`, `out of the way:`, `not readable:`, `to migrate to …:`) so stray value lines no longer fall in, `fixSequences` (consecutive indented commands grouped into ordered runs via an executable allowlist + sudo/env stripping; each step carries `needsAdmin`), `inlineChips` (backticked `brew …` refs from prose — copy-only, never promoted to the primary fix rail), `links` split into `action` / `reference` by host (`developer.apple.com` / `macports.org` are actions; everything else reference), and `rawBody` as the verbatim fallback. Special-case un-indented item collection only for `do not exist:` / `not writable by your user:` (those checks emit dir lists at column 0). **Title can itself be the cue** (e.g. `check_access_directories`'s `Warning: The following directories are not writable…`) — parser arms from the title in that case.
