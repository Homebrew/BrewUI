...
- **The transcript goes to a file, truncated per run** (`~/Library/Logs/Homebrew/self-update.log`, or the run's container under `-uiTesting`). The app is not running while the upgrade is, so stderr goes nowhere anyone will look and a failed self-update would have no account of itself.
