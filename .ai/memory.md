...
## 2026-09-15 — The login-shell script is spelled per shell family (xonsh support)

- **xonsh binds nothing to `-c` trailing arguments.** POSIX shells put them in `$0 $1 …`, fish in `$argv`; xonsh treats `-c <script>` as Python-flavoured xonsh code and leaves the trailing arguments untouched — they are only visible in `sys.argv`, after xonsh's own flags and our `-c <script>` pair. `exec "$0" "$@"` therefore failed with `xonsh: exec: file not found: … $0` for every brew invocation. `LoginShellBrewCommandRunner.shellCommand(for:marker:output:)` now emits `import os, sys; argv = sys.argv[sys.argv.index("-c") + 2:]; print(marker…); os.execvpe(argv[0], argv, ${...}.detype())` for xonsh, which works because the runner builds the argv and the first `-c` is always ours. `${...}.detype()` is the same exported environment `xexec` would pass.
- **The xonsh script must stay in Python mode.** With `-i` xonsh writes a terminal-title escape (`ESC ] 0 ; … BEL`) to stdout before every *subprocess-mode* command whenever `TERM` is set — including one *after* the marker, ahead of the exec, which corrupted `brew info --json=v2` on the pipe path. `$TITLE = None`/`""` still emits an empty escape, so the only clean fix is to avoid subprocess mode: `print` + `os.execvpe` emit nothing. A Finder launch has no `TERM` (the pipe path was accidentally clean there), and the pseudo-terminal path sets `TERM=xterm-256color`, where `TerminalLineAssembler` strips OSC anyway — the Python form makes both paths clean regardless.
- **`-i` stays for xonsh too.** xonsh's `main.py` drops `~/.xonshrc` from the rc list whenever `XONSH_INTERACTIVE` is false, and that file is where xonsh users set `PATH`/Homebrew variables.
- Verified live on xonsh 0.23.8 through the real `BrewCommandService`: `brew --version`, `brew info --json=v2 --installed` (parses), streamed lines start at brew's first line in both output modes, non-zero exit status propagates, rc-file `PATH` reaches the child. Not kept as a test — CI has no xonsh.
