...
- **`brew upgrade --cask homebrew-app` is a `BrewCommand` value, not a command type.** `BrewCommands.selfUpgrade()` builds the argv plus `BrewOperationKind.upgradeApp`, submitted under `BrewOperationID.selfUpgrade`. `PackageOperationSubject.includes` returns `false` for it (grouped with `.maintenance`) — the app's own cask isn't in any inventory a package surface observes.
