...
- **Simulation is now only the DEBUG menu's "Skip the Real Upgrade".** `simulatedUpgradeDuration` became a `@MainActor` closure on `HelperSelfUpdateHandoff`, evaluated when the update is triggered, which is what `RoutingSelfUpdateHandoff` + `DeferredSelfUpdateHandoff` existed for; both are gone. `.selfUpdateAvailable` still covers the simulated handshake; `.selfUpdateRunsBrew` and `.selfUpdateBrewFails` run the real subprocess against the fake `brew` and assert on which outcome alert comes back.
