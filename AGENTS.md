...
3. BrewUILint (matches CI; first build is slow because it compiles `swift-syntax`):
   ...
   - `"$(swift build --package-path Tools/BrewUILint -c release --show-bin-path)/BrewUILint" $(find Homebrew Sources -name '*.swift')`
   - BrewUILint lints `Homebrew` + `Sources` (the whole production tree, all packages); `Tests` is excluded. It must run over all files in one pass — the `nonisolated`-extension rule needs to see every `nonisolated` type declaration to flag a bad extension on it.
